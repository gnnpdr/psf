...
def pad_psf(psf, img):
    ...
    psf_centered = torch.fft.ifftshift(psf_tensor, dim=(-2, -1))
    ...
    psf_padded = F.pad(psf_centered, (pad_left, pad_right, pad_top, pad_bottom), 'constant', 0)
...
def make_trio(file_name, orig_np, blurred_np, res_np, psf_ind, init_noise, optimal_noise):
    ...
    fig.suptitle('Wiener (skimage)', fontsize=14, y=0.78)
    ...
    plt.tight_layout()
    plt.savefig(f'results/restored_trios_wiener_skimage/{file_name}_balance_{optimal_noise}.png', dpi=150, bbox_inches='tight')
    plt.close()
...
def make_save_wiener(img_tensor, psf_tensor, noise, original_name):
    ...
    res_np = wiener(img_np, psf_np, noise)
        
    res_np = np.clip(res_np, 0, 1)
    result_img = (res_np * 255).clip(0, 255).astype(np.uint8)
    Image.fromarray(result_img).save(f'results/restored_wiener_skimage/{original_name}_noise_{noise}_restored.png')
    return res_np
...
os.makedirs('results/restored_wiener_skimage', exist_ok = True)
os.makedirs('results/restored_trios_wiener_skimage', exist_ok = True)
...
for filename in all_files:
    ...
    if os.path.isfile(file_path):
        ...
        #res_np_psnr = make_save_wiener(blurred_tensor, psf_tensor, best_psnr_noise, file_name_psnr)
        #res_np_ssim = make_save_wiener(blurred_tensor, psf_tensor, best_ssim_noise, file_name_ssim)
        ...
        res_np_ssim = make_save_wiener(blurred_tensor, psf_tensor, 0.015, file_name)
        make_trio(file_name, orig_np, blurred_np, res_np_ssim, psf_ind, noise, 0.015)